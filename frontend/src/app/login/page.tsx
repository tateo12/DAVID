...
export default function LoginPage() {
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-surface text-on-surface">
      ...
      <aside className="relative z-20 flex w-full flex-col bg-surface-container-low md:w-[40%]">
        <div className="h-1 w-full bg-gradient-to-r from-primary-container via-secondary-container to-primary-container" />
        <div className="flex flex-1 flex-col justify-center px-8 py-12 lg:px-16">
          ...
          <section className="mb-10">
            <h3 className="mb-2 font-headline text-3xl font-bold tracking-tight">Authorize Access</h3>
            <p className="text-sm text-on-surface-variant">
              Deployment v2.4.0 requires valid credentials to initialize the Command Center.
            </p>
          </section>
          <form
            className="space-y-6"
          ...
          >
            <div>
              <label htmlFor="username" className="mb-2 block font-mono text-[11px] uppercase tracking-wider text-on-surface-variant">
                Username
              </label>
            ...
            <div>
              <label htmlFor="password" className="mb-2 block font-mono text-[11px] uppercase tracking-wider text-on-surface-variant">
                Password
              </label>
              <div className="group relative">
                <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3 text-outline-variant group-focus-within:text-secondary-fixed">
          ...
          <footer className="mt-12 rounded-sm border border-outline-variant/10 bg-surface-container-lowest p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="mb-1 font-mono text-[10px] leading-tight text-on-surface">ACCESS</div>
                <p className="font-mono text-[9px] leading-relaxed text-on-surface-variant">
                  On first database init, create a manager account with{" "}
                  <span className="text-on-surface">SENTINEL_INITIAL_ADMIN_USERNAME</span> and{" "}
                  <span className="text-on-surface">SENTINEL_INITIAL_ADMIN_PASSWORD</span> in{" "}
                  <span className="text-on-surface">backend/.env</span> (see <span className="text-on-surface">.env.example</span>
                  ). Additional users are created by your org&apos;s normal provisioning flow.
                </p>
              </div>
            </div>
          </footer>
          <p className="mt-8 text-center font-mono text-[10px] text-outline">
            <Link href="/register-invite" className="mr-4 text-secondary-fixed hover:underline">
