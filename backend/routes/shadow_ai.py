...
def get_shadow_ai(current_user: dict | None = Depends(get_current_user_optional)) -> list[ShadowAIEvent]:
    eid = None
    if current_user and current_user.get("role") == "employee" and current_user.get("employee_id") is not None:
